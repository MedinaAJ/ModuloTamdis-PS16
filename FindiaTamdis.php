...
class FindiaTamdis extends AdminTab {

	const FILENAME = 'FindiaTamdis';

	public $output = '';
	public $content = '';
	private $module_instance;
	private $ps14_tab = '&tab=FindiaTamdis';
	private $current_controller = 'FindiaTamdis';
	//private $ps14 = true;
	...
	public function __construct($ps14 = true)
	{
		...
		$this->module_instance = Module::getInstanceByName('modulo_tamdis');
	...
	public function displayModuleConfigurationWarning()
	{
		...
		$this->context->smarty->assign(array(
			'tamdis_warning_message' => $this->module_instance->l('Please, first configure your tamdis module as a merchant.', self::FILENAME),
			'module_instance' => $this->module_instance
		));
		$this->content = $this->context->smarty->fetch(_PS_MODULE_DIR_.'modulo_tamdis/views/templates/admin/warning_message.tpl');
	}
	...
	public function displayFormDeliveries($response = null, $detail = null, $error = null)
	{
		...
		$tamdis_order_states = array(
			...
			'4' => $this->module_instance->l('Incident management tamdis', self::FILENAME),
		...
		$img_dir = __PS_BASE_URI__.'modules/modulo_tamdis/img/';
		...
		$this->content .= "<fieldset>
				...
				<div id='tamdis_module' class='$ps_version'>
					<ul class='configuration_menu'>
					  ".((!TamdisLib::getConfigurationField('print_type'))?"<li class='button btnTab".($tab_view == 'labels' ? ' active' : '' )."' tab='label'>
									...
									<thead>
										...
										<tr class='filtros'>
											...
											<td colspan='4'>
												...
												foreach ($tamdis_order_states as $key => $tamdis_order_state)
													$this->content .= "<option value='$key'>$tamdis_order_state</option>";
												$this->content .= "</select>
											</td>
											<td>
				...
				if ($xml->attributes()->NUM[0] != 0)
				{
					...
					foreach ($deliveries_data as $delivery_data)
					{
						...
						foreach ($delivery_data as $key => $delivery_value)
						{
							...
							{
								$this->content .= '<a href="../modules/modulo_tamdis/ajax/createDeliveryNote.php?back='.$back.'&token='.Tools::getValue('token').'&expedition_number='.$delivery_value.'&token='.$token.'&id_employee='.(int)$this->context->cookie->id_employee.'">
							...
							if ($key == 'Detalles')
								$this->content .= '<a class="verDetalles" href="'.__PS_BASE_URI__.'modules/modulo_tamdis/ajax/getExpeditionAjax.php?expedition_number='.$delivery_number.'&token='.$token.'&id_employee='.(int)$this->context->cookie->id_employee.'">
		...
		$this->content .= '</table>
			...
			<li id="packing_list"'.( $tab_view == 'packing_list' ? '  class="default"' : '' ).'>
				...
				<tbody>
					<tr>
						<td>
							<a href="../modules/modulo_tamdis/ajax/createPackingList.php?back='.$back.'&token='.Tools::getValue('token').'&id_employee='.$this->context->cookie->id_employee.'" target="_blank">
		...
		if (TamdisLib::getConfigurationField('pickup') == 1)
			$steady_pickup = true;
			...
			$this->content .= '<tbody>
						<tr>
							...
							<p><img src="../img/admin/help2.png" /> 
							   '.$this->module_instance->l('14H is past, to create a pickup please contact tamdis on 902101010 or via ', self::FILENAME).'
							</p>
							<p><a href="http://www.tamdis.com" target="_blank">www.tamdis.com</a></p>
						...
						<script type="text/javascript">
							$(document).ready(function(){
								...
								$(\'#labelsFilter\').click(function(){
										  ...
										  $.ajax(
												{
													url: \''._MODULE_DIR_.'modulo_tamdis/ajax/getLabelAjax.php\',
													data: params,
													type: \'POST\',
													async: true,
													dataType: \'json\',
	...
	{
		$sql = 'SELECT o.reference,o.id_order,o.date_add,o.reference, a.firstname, a.lastname, a.address1, a.address2, a.postcode, a.city, cl.name as country, s.name as state FROM '._DB_PREFIX_.'orders o INNER JOIN '._DB_PREFIX_.'tamdis_order so ON so.id_order  = o.id_order  INNER JOIN '._DB_PREFIX_.'address a ON o.id_address_delivery = a.id_address INNER JOIN '._DB_PREFIX_.'country_lang cl ON cl.id_country = a.id_country AND cl.id_lang ='.(int)$this->context->language->id.' LEFT JOIN '._DB_PREFIX_.'state s ON s.id_state = a.id_state ';
		$where = array();
		if ((int)$id > 0)
	...
	private function getLabelData($id_order)
	{
					...
					//$carrier_pos = TamdisLib::getSeurCarrier('SEP');
					$datospos = '';
	...
	public static function uninstalltamdisCashOnDelivery()
	{	
		if ($module = Module::getInstanceByName('tamdiscashondelivery'))
		{
			$module_dir = _PS_MODULE_DIR_.str_replace(array('.', '/', '\\'), array('', '', ''), $module->name);
			Admintamdis::recursiveDeleteOnDisk($module_dir);
		}
			
		return true;
	}
	
	public static function installtamdisCashOnDelivery()
	{
		if (Admintamdis::moveFiles())
			return true;
	...
	{
		if (!is_dir(_PS_MODULE_DIR_.'tamdiscashondelivery'))
		{
			$module_dir = _PS_MODULE_DIR_.str_replace(array('.', '/', '\\'), array('', '', ''), 'tamdiscashondelivery');
			Admintamdis::recursiveDeleteOnDisk($module_dir);
		}
		$dir = _PS_MODULE_DIR_.'tamdis/install/tamdiscashondelivery';
		if (!is_dir($dir))
			return false;

		Admintamdis::copyDirectory($dir, _PS_MODULE_DIR_.'tamdiscashondelivery');
			...
			foreach ($objects as $object)
				if ($object != '.' && $object != '..')
				{
					if (filetype($dir.'/'.$object) == 'dir')
						Admintamdis::recursiveDeleteOnDisk($dir.'/'.$object);
					else
						unlink($dir.'/'.$object);
				}
			reset($objects);
