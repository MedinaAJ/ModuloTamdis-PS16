...
include_once(_PS_MODULE_DIR_.'modulo_tamdis/classes/pdf.php');
...
class modulo_tamdis extends Module
{
	...
	/*	$success = true;
		...
		$success &= Configuration::deleteByName('tamdis_URLWS_AUT');
		$success &= Configuration::deleteByName('tamdis_URLWS_LABEL');
		$success &= Configuration::deleteByName('tamdis_WS_USER');
		$success &= Configuration::deleteByName('tamdis_WS_PASS');
	...
	private function createAdminTab()
	{
		...
		foreach (Language::getLanguages() as $language)
			$tab->name[$language['id_lang']] = 'tamdis B2C';

		$tab->class_name = 'AdminFindia';
	...
	{
		$id_tab = (int)Tab::getIdFromClassName('Findia');
	...
	public function displayForm()
	{
		...
		$fields_form[0]['form'] = array(
			...
			//'input' => array(
				...
				array(
					'type' => 'text',
					'label' => $this->l('ACCOUNT tamdis: '),
					'name' => 'n_account',
					'desc' => $this->l('Introduce la CUENTA de tamdis'),
					'size' => 150,
					'required' => true
				)
			),
	...
	public function hookAdminOrder($params)
	{
		...
		if (!TamdisLib::getdataOrder((int)$order->id) && Tools::getValue('npackages')=='' && Tools::getValue('weight')==''){
			$order_data = TamdisLib::getdataInicio((int)$order->id);
			
		}else{
			if (Tools::getValue('npackages') && Tools::getValue('weight')){
				TamdisLib::setdataOrder((int)$order->id);
				$order_data = TamdisLib::getdataOrder((int)$order->id);
			}else{
				$order_data = TamdisLib::getdataOrder((int)$order->id);
			}
		}
		...
		if (Tools::getValue('Labeltamdis'))
		{
			...
			if ($success === true)
			{
				if (!$this->setAsPrinted((int)$order->id))
					...
					$this->printFile($label_data['name'].'_'.$label_data['lastname']);
				}
			}
			else
		...
		$sq="Select * from ps_megaproductcart where id_cart=".$order->id_cart." and id_product=481 and attributes like '%1923%'" ;
		if (Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sq)){
			$montag=true;
		}
	...
	private function printFile($name)
	{	
